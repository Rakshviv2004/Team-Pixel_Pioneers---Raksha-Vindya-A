...
export default function DashboardPage({ navigate }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (c && c.activities) setActivities((c.activities || []) as Array<{ id: number; type: string; description: string; created_at: string }>)
      if (sv && sv.resources) setSaved(sv.resources)
      if (ev && ev.events) setEvents(ev.events)
    }).finally(() => setLoading(false))
  }, [])

  const statCards = [
    { label: 'Resources added', value: String(stats?.resources_added ?? 0), unit: 'by you', color: '#57e57f', icon: '＋' },
    { label: 'Items repaired', value: String(stats?.items_repaired ?? 0), unit: 'community total', color: '#1acbae', icon: '🔧' },
    { label: 'Waste diverted', value: `${stats?.waste_diverted_kg ?? 0}kg`, unit: 'estimated', color: '#bd8c2a', icon: '♻' },
    { label: 'Carbon saved', value: `${stats?.carbon_saved_kg ?? 0}kg`, unit: 'CO₂ equiv.', color: '#185acb', icon: '🌱' },
  ]
